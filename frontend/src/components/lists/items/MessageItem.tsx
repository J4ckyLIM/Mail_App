...
const MessageItem: FC<MessageItemProps> = ({ message, onClick }) => {
  ...
  return (
    ...
    >
      ...
      <VStack flex="60%" alignItems="flex-start" spacing="0" pl="2" opacity={opacity}>
        <Text fontWeight="bold" fontSize="md">{message.writtenBy}</Text>
        <Text fontWeight={titleFontWeight} fontSize="sm">{message.title}</Text>
      </VStack>
      <Text flex="20%" opacity={opacity} pr="2" fontSize="sm" fontWeight="semibold" textAlign="right">{convertDateToReadable(message.sentAt)}</Text>
    </HStack>
  );
};
