...
describe('UserService', () => {
  ...
  describe('create', () => {
    it('should create a new user', async () => {
      const userProps = { name: 'example', email: 'example@gmail.com', password: 'example' };
    ...
    it('should throw an error if the email of the user already exists', async () => {
      ...
      await expect(userService.create(userProps)).rejects.toThrowError(`User with email ${userProps.email} already exists`);
  ...
  describe('update', () => {
    it('should update an existing user', async () => {
      ...
      const existingUser = new User({ name: 'example', email: 'example@gmail.com', password: 'example' });

      const { id } = existingUser;

      jest.spyOn(userRepository, 'findOneBy').mockResolvedValue(existingUser);
      jest.spyOn(userRepository, 'update').mockResolvedValue(undefined);
      jest.spyOn(userRepository, 'findOneBy').mockResolvedValue({ ...existingUser, ...updateProps } as User);
    ...
    it('should throw an error if the user id does not exist', async () => {
      const fakeId = '12345';
      const updateProps = { name: 'updatedName', email: 'updatedEmail@gmail.com', password: 'updatedPassword' };

      jest.spyOn(userRepository, 'findOneBy').mockResolvedValue(undefined);

      await expect(userService.update(fakeId, updateProps)).rejects.toThrowError(`User with id ${fakeId} not found`);
